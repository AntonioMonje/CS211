...
void printAllStudents(vector<Student>& v);
...
void printAllStudents(vector<Student>& v)
{
  ...
  for(int n = 0; n < v.size(); n++)
    {
      cout << "\n" << setw(10) << v[n].getId() << setw(10) << v[n].getFirstName() << setw(10) << v[n].getLastName() << right << setw(7) << v[n].getDollar() << ".";
...
void addCourseToStudent(vector<Student>& v)
{
  ...
  if(!found)
    ...
    {
      cout << "Course was added to " << v[k].getFirstName();
      v[k].chargeFee(50, 10);
    }
...
void dropCourseFromStudent(vector<Student>& v)
{
  ...
  int crn = getNumberInRange(1, 99999, "Invalid CRN. Enter the CRN: ");
	...
	cout << "No student found" << endl;
	  ...
	  {
	    cout << "student not enrolled in the class" << endl;
	  }
   else if(found)
      {
	cout << "Student has been dropped" << endl;
	v[k].reduceFee(50, 10);
      }
