...
bool palindrome(LL<char> &l1, LL<char> &l2);
...
try
  ...
  if(palindrome(l1, l2))//checks if its a palindrome if yes show message
...
bool palindrome(LL<char> &l1, LL<char> &l2)
{
  while(!l1.isEmpty())//while not empty check that the first element is the same
    {
      if(l1.deleteFront() != l2.deleteRear())//checks if their different
